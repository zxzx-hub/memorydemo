...
`docs/agent-memory-design.html`，尤其是 TenantContext、三层记忆、Consolidate Once、
...
- **修改模块**：`app/auth`、`app/api/dependencies.py`、`app/core`、`app/infrastructure/db`、`app/infrastructure/redis`、`app/main.py`。
...
- **修改模块**：`app/domain`、`app/ports/repositories.py`、`app/ports/unit_of_work.py`、`app/infrastructure/db/models`、`app/infrastructure/db/repositories`、`migrations`。
...
- **修改模块**：`app/services/memory_service.py`、事件/工作记忆 Repository、exact idempotency 支持、API 路由。
...
- **修改模块**：`app/services/consolidate_once.py`、checkpoint/evidence/candidate Repository、UoW、checkpoint prompt。
...
- **修改模块**：`app/services/candidate_governance.py`、长期主记录/版本 Repository、`JobDispatcher`。
...
- **修改模块**：`app/services/retrieval_service.py`、`context_compiler.py`、exact/vector/graph Port 与适配器。
...
- **修改模块**：`app/services/lifecycle_service.py`、生命周期/配额/删除 Repository、各索引清理适配器。
...
- **修改模块**：`app/ports/llm_client.py`、`app/infrastructure/llm`、`app/prompts`。
